...
from torchode.repara import _Repara, PCHIPRepara, PCHIPNonUniformRepara
...
def resample_with_pchip(
    ...
    x_features = sparse_list_X[0].shape[-1]  # 4
    sc_features = sparse_list_SC[0].shape[-1]  # 4
    y_features = sparse_list_Y[0].shape[-1]  # 1
    
    full_X = torch.zeros(batch_size, target_len, x_features, device=device)
    full_SC = torch.zeros(batch_size, sc_features, device=device)
    full_Y = torch.zeros(batch_size, target_len, y_features, device=device)
    
    print(f"Step 3: Starting **FIXED** resampling for {batch_size} samples to length {target_len}...")
    
    for i in range(batch_size):
        if (i + 1) % 100 == 0 or i == 0:
            print(f"Resampling progress: sample {i+1}/{batch_size} ({(i+1)/batch_size*100:.1f}%)")
        
        sparse_X_i = sparse_list_X[i]  # [sparse_len, 4]
        ...
        if sparse_len < 2:
            # 太稀疏，无法插值，重复首点或跳过
            if sparse_len == 1:
                full_X[i] = sparse_X_i[0:1].repeat(target_len, 1)
                full_Y[i, :, 0] = sparse_Y_i[0, 0].repeat(target_len)
            continue
        
        # --- 核心修改：基于时间 t 进行插值 ---
        t_sparse = sparse_X_i[:, 0].contiguous() # 真实时间 t，[sparse_len]
        # 扩展 t_sparse 用于传入插值器
        t_sparse_expanded = t_sparse.unsqueeze(0).unsqueeze(-1) # [1, sparse_len, 1]
        
        t_min, t_max = t_sparse[0], t_sparse[-1]
        t_query = torch.linspace(t_min, t_max, target_len, device=device) # 目标查询时间
        
        # --- Step 1: 预处理和 SC ---
        full_SC[i] = sparse_list_SC[i]
        full_X[i, :, 0] = t_query # t 轴直接赋值
        
        # --- Step 2: 插值 X (I/T/U) ---
        for ch in range(1, x_features): # 从 I (ch=1) 开始
            sparse_ch = sparse_X_i[:, ch].unsqueeze(-1) # 被插值量 [sparse_len, 1]
            sparse_T = sparse_ch.unsqueeze(0)           # y_vals: [1, sparse_len, 1]
            
            # 关键修改：将 y_vals 和 t_vals 传入 __init__
            repara = PCHIPNonUniformRepara(sparse_T, t_sparse_expanded)
            ...
            for k in range(target_len):
                ...
                # 查找 t_q 所在的区间 [t_j, t_{j+1}]
                # searchsorted 返回第一个大于等于 t_q 的索引 j+1
                j = torch.searchsorted(t_sparse, t_q, right=False).item() - 1
                
                # 确保 j 在有效区间 [0, sparse_len-2] 内
                j = int(j)
                
                t_j = t_sparse[j]
                h_j = t_sparse[j+1] - t_j # 真实时间步长
                
                # 计算局部规范化步长 s = (t_q - t_j) / h_j
                s_local = (t_q - t_j) / h_j
                s_local = torch.clamp(s_local, 0.0, 1.0)
                
                phi, _ = repara.forward(s_local.unsqueeze(0), j)
                full_X[i, k, ch] = phi.item()
        
        # --- Step 3: 插值 Y (SOC) ---
        sparse_Y_i = sparse_list_Y[i] # [sparse_len, 1]
        sparse_Y_flat = sparse_Y_i[:, 0].unsqueeze(-1) # [sparse_len, 1]
        sparse_T = sparse_Y_flat.unsqueeze(0)          # y_vals: [1, sparse_len, 1]
    ...
    # Clamp: Y SOC [0,1]；X t(0)/T(2)/U(3) >=0, I(1) 无
    full_Y = torch.clamp(full_Y, min=0.0, max=1.0)
    full_X[:, :, 0] = torch.clamp(full_X[:, :, 0], min=0.0)  # t
    full_X[:, :, 2] = torch.clamp(full_X[:, :, 2], min=0.0)  # T
    full_X[:, :, 3] = torch.clamp(full_X[:, :, 3], min=0.0)  # U
    
    print("Step 3: Resampling completed. Shapes: X {}, SC {}, Y {}".format(
        full_X.shape, full_SC.shape, full_Y.shape))
    print(f"Post-clamp Y min/max/mean: {full_Y.min().item():.6f} / {full_Y.max().item():.6f} / {full_Y.mean().item():.6f}")
    print(f"Post-clamp X min (t/I/T/U): [{full_X[:,:,0].min():.3f}, {full_X[:,:,1].min():.3f}, {full_X[:,:,2].min():.3f}, {full_X[:,:,3].min():.3f}]")
    print("\n")
...
def load_preprocessed_data(dataset: str, model_type: type, device: torch.device) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """加载预处理后的数据，如果文件存在直接加载，否则返回 None"""
    preprocessed_dir = Path(f"datasets/preprocessed/{dataset}")
    preprocessed_file = preprocessed_dir / f"{model_type.__name__}_preprocessed.pkl"
    
    if preprocessed_file.exists():
        print(f"Loading preprocessed data from: {preprocessed_file}")  # 日志：加载预处理数据文件
        with open(preprocessed_file, "rb") as f:
            data_dict = pickle.load(f)
        X = torch.from_numpy(data_dict["X"]).to(device)
        SC = torch.from_numpy(data_dict["SC"]).to(device)
        Y = torch.from_numpy(data_dict["Y"]).to(device)
        print(f"Preprocessed data loaded successfully. Shapes - X: {X.shape}, SC: {SC.shape}, Y: {Y.shape}")  # 日志：预处理数据加载成功
        return X, SC, Y
    else:
        print(f"Preprocessed file not found: {preprocessed_file}. Will perform preprocessing.")  # 日志：未找到预处理文件，将执行预处理
        return None, None, None

def save_preprocessed_data(X: torch.Tensor, SC: torch.Tensor, Y: torch.Tensor, dataset: str, model_type: type):
    """保存预处理后的数据到 pickle 文件"""
    preprocessed_dir = Path(f"datasets/preprocessed/{dataset}")
    preprocessed_dir.mkdir(parents=True, exist_ok=True)  # 创建目录
    preprocessed_file = preprocessed_dir / f"{model_type.__name__}_preprocessed.pkl"
    
    data_dict = {
        "X": X.cpu().numpy(),  # 转回 numpy 保存
        "SC": SC.cpu().numpy(),
        "Y": Y.cpu().numpy()
    }
    with open(preprocessed_file, "wb") as f:
        pickle.dump(data_dict, f)
    print(f"Preprocessed data saved to: {preprocessed_file}")  # 日志：预处理数据已保存
...
def main(
    ...
    # 首先尝试加载预处理数据
    X, SC, Y = load_preprocessed_data(dataset, model_type, device)
    
    using_preprocessed = X is not None
    if using_preprocessed:
        print("Using preprocessed data: Yes (skipping drop and resample steps).")  # 日志：是否直接使用预处理数据 - 是
    else:
        print("Using preprocessed data: No (performing drop and resample preprocessing).")  # 日志：是否直接使用预处理数据 - 否，将重新预处理
        
        # 如果未找到预处理文件，则加载原始数据并执行预处理
        # 加载原始数据
        X_raw, SC_raw, Y_raw = load_data(dataset=dataset, model_type=model_type, device=device)
        
        # 执行随机点删除
        X_list, SC_list, Y_list = drop_random_in_range_points(X_raw, SC_raw, Y_raw, drop_range=(0.10, 0.14))
        
        # 执行插值重采样
        target_len = X_raw.shape[1]  # e.g., 50
        X, SC, Y = resample_with_pchip(X_list, SC_list, Y_list, target_len=target_len, device=device)
        
        # 保存预处理后的数据（仅首次执行）
        save_preprocessed_data(X, SC, Y, dataset, model_type)
    print("\n")  # 日志：空行分隔
    ...
    for model_sel in [W_SOCNet]:
        for data_sel in ["ordered"]:
            main(
                ...
                max_epochs=800,  # 按需要调大训练的 epoch 数目，实现训练效果
            )